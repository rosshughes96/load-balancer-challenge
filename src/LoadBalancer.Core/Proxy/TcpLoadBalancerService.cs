...
namespace LoadBalancerProject.Proxy
{
    ...
    /// and forwards them to a selected healthy backend using the IRequestForwarder.
    /// Works at L4 (protocol-agnostic). Metrics & structured logs are emitted by the forwarder.
    /// </summary>
    public class TcpLoadBalancerService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            listener.Start();
            _logger.LogInformation("TCP LB listening on port {Port}", _listenPort);
