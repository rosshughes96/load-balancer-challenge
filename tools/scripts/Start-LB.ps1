...
param(
  ...
  [int[]]$Delays = @(1000,5000,5000,5000,5000),
...
# PIDs will be saved so Stop-LB.ps1 can kill them
$pidsFile = Join-Path $PSScriptRoot "RunState_pids.json"
$pids = @()
...
$pids += @{ role = "api"; port = $ApiPort; pid = $apiProc.Id }

# Save the run state
$pids | ConvertTo-Json | Out-File -FilePath $pidsFile -Encoding UTF8
Write-Host "`nSaved PIDs to $pidsFile" -ForegroundColor Yellow
